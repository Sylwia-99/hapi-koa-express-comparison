    ...
    const plane = await Plane.findOrCreate({
      where: {
        ...
        segments_equipment_description:
          req.payload.segments_equipment_description,
      },
    });
...
exports.update = async (req, res) => {
  ...
  try {
    ...
    const flightDetails = await FlightDetails.update(req.payload, {
      where: { fk_flight_id: ticket.flight_id },
    });
    const plane = await Plane.update(req.payload, {
      where: { plane_id: ticket.Flight.plane_id },
    });
    const airline = await Airline.update(req.payload, {
      where: { airline_id: ticket.Flight.Plane.airline_id },
    });
    const starting_airport = await Airport.update(req.payload, {
      where: { fk_flight_id: ticket.Flight.starting_airport.airport_id },
    });
    const destination_airport = await Airport.update(req.payload, {
    ...
    return ticket &&
      ...
      : Boom.badRequest(`Cannot update Ticket with id=${id}.`);
  } catch (err) {
    throw Boom.badRequest(err.message);
  }
};
...
exports.delete = async (req, res) => {
  ...
  try {
    ...
    return ticketToDelete === 1
      ...
      : Boom.notFound(`Cannot delete Ticket with id=${id}.`);
  } catch (err) {
    throw Boom.badRequest(err.message);
  }
};
