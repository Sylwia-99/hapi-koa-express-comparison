...
exports.getAll = async (ctx, next) => {
  try {
    const tickets = await Ticket.find({"total_fare":  563.75}).limit(1000)
    return  tickets ? ctx.body = tickets : ctx.throw(400, `Cannot find Tickets.`);
  }
  catch(err) {
    return ctx.throw(err.statusCode || 500, err.message);
  }
...
exports.get = async (ctx, next)=>{
  ...
  try {
    ...
    return ticket ? ctx.body = ticket : ctx.throw(400, `Cannot find Ticket with id=${id}.`);
  }
  catch(err) {
    return ctx.throw(err.statusCode || 500, err.message);
    }
...
exports.update = async (ctx, next, body) => {
  ...
  try {
    ...
    return ticket ? ctx.body = "Ticket was updated successfully." : ctx.throw(400, `Cannot update Ticket with id=${id}.`);
  }
  catch(err) {
...
exports.delete = async (ctx, next) => {
  ...
  try {
    const result  = await Ticket.deleteOne(
      {_id: id});
      return result.deletedCount === 1 ? ctx.body = "Ticket was deleted successfully!" : ctx.throw(400, `Cannot delete Ticket with id=${id}.`);
  }
  catch(err) {
    return ctx.throw(err.statusCode || 500, err.message);
    }
